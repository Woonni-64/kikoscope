...
export default function IeltsFlashcardsPage() {
  ...
  const currentAudioKey = currentWord?.english.toLowerCase() ?? "";
  const currentAudioUrl = currentAudioKey ? audioUrls[currentAudioKey] : undefined;
  ...
  useEffect(() => {
    if (!currentWord || audioUrls[currentAudioKey] !== undefined) return;

    const loadAudio = async () => {
      try {
        const response = await fetch(`https://api.dictionaryapi.dev/api/v2/entries/en/${encodeURIComponent(currentWord.english)}`);
        if (!response.ok) throw new Error("No audio");
        const data = (await response.json()) as { phonetics?: { audio?: string }[] }[];
        const audioUrl = data?.[0]?.phonetics?.[0]?.audio || data?.[0]?.phonetics?.find((item) => item.audio)?.audio || null;
        setAudioUrls((prev) => ({ ...prev, [currentAudioKey]: audioUrl || null }));
      } catch {
        setAudioUrls((prev) => ({ ...prev, [currentAudioKey]: null }));
      }
    };

    void loadAudio();
  }, [audioUrls, currentAudioKey, currentWord]);
                ...
                <div className="flip-card h-80 mb-8">
                  <div className={`flip-card-inner ${isFlipped ? "flipped" : ""}`}>
                    <button
                      ...
                      className="flip-card-front flashcard relative w-full p-8 flex flex-col items-center justify-center cursor-pointer text-center"
                    >
                      <span className={`absolute left-6 top-6 inline-block px-3 py-1 rounded-full text-xs font-medium ${categoryColors[currentWord.category]}`}>
                        {activeSource === "builtin" ? categoryLabels[currentWord.category] : sourceLabel}
                      </span>
                      <span className="absolute right-6 top-6 text-xs text-[#8D7B6B]">{currentIndex + 1}/{totalWords}</span>
                      <div className="flex min-h-[190px] flex-col items-center justify-center">
                        <h2 className="sf mb-4 text-center text-5xl font-bold text-[#3E2723]">{currentWord.english}</h2>
                        <div className="flex items-center justify-center gap-2 text-[#8D7B6B]">
                          <span>{currentWord.phonetic}</span>
                          {currentAudioUrl && <audio ref={audioRef} src={currentAudioUrl} preload="none" />}
                          <button
                            type="button"
                            aria-label={`播放 ${currentWord.english} 的发音`}
                            ...
                            className={`inline-flex h-7 w-7 items-center justify-center rounded-full transition ${
                              currentAudioUrl
                                ? "text-[#8D7B6B] hover:bg-[#FDFBF7] hover:text-[#8B5E3C]"
                                : "cursor-not-allowed text-[#CBBBAA] opacity-60"
                            }`}
                          >
                            <SpeakerIcon />
                          </button>
                          {currentAudioUrl === null && <span className="text-xs text-[#8D7B6B]">暂无发音</span>}
                        </div>
                      </div>
                      {currentWord.sourceLabel && <p className="absolute bottom-6 text-xs text-[#8D7B6B]">来自：{currentWord.sourceLabel}</p>}
                    </button>
